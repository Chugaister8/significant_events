...
class RiskDirectionFrame(tk.Frame):
    """
    Сторінка з усіма індикаторами одного напрямку ризику.
    Показує зведений дашборд та список IndicatorFormFrame.
    """

    def __init__(
        self,
        parent:    tk.Misc,
        direction: dict,           # елемент RA_DIRECTIONS
        period:    str,
        enterprise: str,
        saved_data: dict,          # {code: {values, level, notes}}
        on_save: Callable[[str, str, str, dict], None],  # dir_key, period, ent, data
        ...
        self._levels: dict[str, str] = {}    # code → level
    ...
    def _build(self) -> None:
        ...
        title_f.grid(row=0, column=2, sticky="ew", padx=14, pady=8)
        ...
        ctrl_f.grid(row=0, column=3, sticky="e", padx=12, pady=8)

        tk.Label(ctrl_f, text="Підприємство:",
                 bg=C["bg_header"], fg=C["text_muted"],
                 font=FONT_SMALL).pack(side="left")
        self._ent_var = tk.StringVar(value=self._enterprise)
        ent_entry = make_dark_entry(ctrl_f, width=20)
        ent_entry.insert(0, self._enterprise)
        ent_entry.configure(textvariable=self._ent_var)
        ent_entry.pack(side="left", padx=(4, 12))

        tk.Label(ctrl_f, text="Квартал:",
                 bg=C["bg_header"], fg=C["text_muted"],
                 font=FONT_SMALL).pack(side="left")
        self._period_var = tk.StringVar(value=self._period)
        period_cb = make_dark_combo(ctrl_f,
                                     values=self._gen_periods(), width=10)
        period_cb.set(self._period)
        period_cb.configure(textvariable=self._period_var)
        period_cb.pack(side="left", padx=(4, 12))
        period_cb.bind("<<ComboboxSelected>>", lambda _: self._reload_period())
        ...
        # ── Summary dashboard ────────────────────────────────────────────
        self._dash_frame = tk.Frame(self, bg=C["bg_main"])
        self._dash_frame.grid(row=1, column=0, sticky="ew", padx=12, pady=(8, 0))
    ...
    def _build_dashboard(self) -> None:
        ...
        # summary strip
        summary_f = tk.Frame(self._dash_frame, bg=C["bg_surface"])
        summary_f.pack(fill="x", pady=(0, 6))
        summary_f.columnconfigure(list(range(6)), weight=1)

        tk.Label(summary_f, text="Зведення ризик-апетиту:",
                 bg=C["bg_surface"], fg=C["text_muted"],
                 font=FONT_SMALL_BOLD).pack(side="left", padx=14, pady=8)
        ...
        for lvl in ("Green", "Yellow", "Orange", "Red"):
            ...
            if cnt:
                badge = tk.Label(summary_f,
                                  text=f"  ● {RA_LABELS[lvl]}: {cnt}  ",
                                  bg=RA_COLORS[lvl], fg="white",
                                  font=FONT_SMALL_BOLD, pady=3)
                badge.pack(side="left", padx=4, pady=6)

        nd = total - defined
        if nd:
            tk.Label(summary_f, text=f"  Не заповнено: {nd}  ",
                     bg=C["border_soft"], fg="white",
                     font=FONT_SMALL_BOLD, pady=3).pack(side="left", padx=4, pady=6)

        # Aggregate level
        agg_lbl = tk.Label(summary_f,
                            text=f"  Агрегований рівень: {RA_LABELS.get(agg, '—')}  ",
                            bg=RA_COLORS.get(agg, C["border_soft"]),
                            fg="white", font=FONT_BOLD, pady=3)
        agg_lbl.pack(side="right", padx=14, pady=6)

        # Progress
        pct = int(defined / total * 100) if total else 0
        prog_f = tk.Frame(self._dash_frame, bg=C["bg_main"])
        prog_f.pack(fill="x", pady=(0, 4))
        tk.Label(prog_f, text=f"Заповнено {defined}/{total} індикаторів ({pct}%)",
                 bg=C["bg_main"], fg=C["text_muted"],
                 font=FONT_SMALL).pack(side="left", padx=14)

        bar_bg = tk.Frame(prog_f, bg=C["bg_surface"], height=6, width=300)
        bar_bg.pack(side="left", padx=8, pady=4)
        bar_bg.pack_propagate(False)
        if pct > 0:
            bar_fill = tk.Frame(bar_bg,
                                 bg=RA_COLORS.get(agg, C["accent"]),
                                 height=6, width=int(300 * pct / 100))
    ...
    def _build_indicators(self) -> None:
        ...
        for spec in indicators:
            ...
            if spec["group"] != current_group:
                ...
                gf.grid(row=row, column=0, sticky="ew",
                        padx=12, pady=(12, 2))
                tk.Frame(gf, bg=self._direction["color"], width=20, height=2).pack(
                    side="left")
            ...
            saved_vals = saved_code.get("values", {})
            saved_note = saved_code.get("notes", "")

            frm = IndicatorFormFrame(
                parent=self._list_frame,
                spec=spec,
                saved_values=saved_vals,
        ...
        # padding
        tk.Frame(self._list_frame, bg=C["bg_main"], height=20).grid(
            row=row, column=0)
    ...
    def _reload_period(self) -> None:
        # In real app: load saved data for new period; here just refresh
        self._period = self._period_var.get()
        self._status_lbl.configure(
            text=f"Квартал змінено на {self._period}. Введіть дані.")
    ...
    def _save(self) -> None:
        ...
        self._status_lbl.configure(
            text=f"Збережено · {ent} · {period} · {datetime.now().strftime('%H:%M:%S')}")
...
class RiskAppetitePage(tk.Frame):
    ...
    4 великі кнопки (2×2) → переходять до RiskDirectionFrame.
    ...
    def __init__(self, master: tk.Misc, **kwargs) -> None:
        ...
        self._all_data: dict = {}      # {dir_key: {period: {ent: {code: {values, level}}}}}
        self._current_dir_frame: RiskDirectionFrame | None = None
        self._load_data()
        self._show_menu()
    ...
    def _show_menu(self) -> None:
        """Показати головний екран з 4 кнопками."""
        ...
        wrapper = tk.Frame(self, bg=C["bg_main"])
        wrapper.place(relx=0.5, rely=0.5, anchor="center")

        # Title
        tk.Label(wrapper, text="РИЗИК АПЕТИТ",
                 bg=C["bg_main"], fg=C["accent_muted"],
                 font=("Arial", 18, "bold")).grid(
            row=0, column=0, columnspan=2, pady=(0, 6))
        tk.Label(wrapper,
                 text="Оберіть напрямок для введення та перегляду індикаторів",
                 bg=C["bg_main"], fg=C["text_muted"],
                 font=("Arial", 10)).grid(
            row=1, column=0, columnspan=2, pady=(0, 24))

        # Summary mini-badges for all directions
        sum_f = tk.Frame(wrapper, bg=C["bg_main"])
        sum_f.grid(row=2, column=0, columnspan=2, pady=(0, 28))

        for dr in RA_DIRECTIONS:
            agg = self._direction_agg(dr["key"])
            bg  = RA_COLORS.get(agg, COLORS["border_soft"])
            tk.Label(sum_f,
                     text=f"  {dr['icon']} {agg}  ",
                     bg=bg, fg="white", font=FONT_TINY).pack(side="left", padx=4)

        # 2×2 grid of direction buttons
        btn_grid = tk.Frame(wrapper, bg=C["bg_main"])
        btn_grid.grid(row=3, column=0, columnspan=2)

        for i, dr in enumerate(RA_DIRECTIONS):
            r = i // 2
            c = i % 2
            self._build_dir_button(btn_grid, dr, r, c)

        # Bottom hint
        tk.Label(wrapper,
                 text="Дані автоматично зберігаються при натисканні «Зберегти»",
                 bg=C["bg_main"], fg=C["text_subtle"],
                 font=FONT_TINY).grid(
            row=4, column=0, columnspan=2, pady=(24, 0))

    def _direction_agg(self, dir_key: str) -> str:
        """Агрегований рівень по всьому напрямку (всі квартали, всі підприємства)."""
        all_levels: list[str] = []
        for period_data in self._all_data.get(dir_key, {}).values():
            for ent_data in period_data.values():
                for code_data in ent_data.values():
                    lvl = code_data.get("level", "—")
                    if lvl != "—":
                        all_levels.append(lvl)
        return aggregate_level(all_levels) if all_levels else "—"

    def _build_dir_button(
        self, parent: tk.Misc, dr: dict, r: int, c: int
    ) -> None:
        C     = COLORS
        color = dr["color"]
        agg   = self._direction_agg(dr["key"])
        ...
        card = tk.Frame(parent, bg=C["bg_surface"],
                        cursor="hand2", width=280, height=160)
        ...
        # Top accent strip
        tk.Frame(card, bg=color, height=4).grid(
            row=0, column=0, sticky="ew")

        inner = tk.Frame(card, bg=C["bg_surface"])
        inner.grid(row=1, column=0, sticky="nsew", padx=20, pady=14)
        inner.columnconfigure(0, weight=1)

        # Icon + title
        tk.Label(inner, text=dr["icon"],
                 bg=C["bg_surface"], fg=color,
                 font=("Arial", 22)).grid(row=0, column=0, sticky="w")
        ...
        tk.Label(inner, text=dr["desc"],
                 bg=C["bg_surface"], fg=C["text_muted"],
                 font=FONT_TINY).grid(row=2, column=0, sticky="w")

        # Status badge
        status_f = tk.Frame(card, bg=C["bg_surface"])
        status_f.grid(row=2, column=0, sticky="ew", padx=12, pady=(0, 10))
        tk.Label(status_f,
                 text=f"  Статус: {RA_LABELS.get(agg, '—')}  ",
                 bg=agg_c, fg="white", font=FONT_TINY).pack(side="left")
        ...
        for w in [card, inner] + list(inner.winfo_children()) + [status_f]:
            w.bind("<Button-1>", lambda _, fn=_open: fn())
            w.configure(cursor="hand2")

        # Hover effects
        def _enter(_: object, f=card, co=color) -> None:
    ...
    def _open_direction(self, dir_key: str) -> None:
        ...
        # pick most recent saved period / enterprise for this direction
