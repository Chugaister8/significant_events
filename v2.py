...
class EventDetailWindow:
    ...
    Відкривається при подвійному кліку по рядку таблиці.
    """

    def __init__(self, parent_root, record, all_records, save_callback, delete_callback, toast_callback):
        """
        parent_root    — головне вікно (для центрування)
        record         — кортеж даних запису
        all_records    — посилання на список усіх записів (для оновлення)
        save_callback  — викликається після збереження змін: save_callback(old_id_str, new_record)
        delete_callback— викликається після видалення: delete_callback(id_str)
        toast_callback — показати toast повідомлення
        """
    ...
    def _build_window(self):
        ...
        self.win.grab_set()  # модальне вікно
        ...
        # Кольорова смуга зліва (за типом ризику)
    ...
    def _info_row(self, parent, label, value, row, col=0):
        ...
        cell.grid(row=row, column=col, sticky="nsew", padx=(16 if col == 0 else 6, 6 if col == 0 else 16), pady=3)
        ...
        tk.Label(cell, text=value or "—", bg=C["bg_card"], fg=C["text_primary"],
                 font=("Segoe UI", 9), wraplength=280, justify="left").grid(row=1, column=0, sticky="w", pady=(2, 0))
    ...
    def _build_view_content(self):
        """Побудова вмісту у режимі перегляду"""
        C = COLORS
        rec = self.record
        ...
        priority_colors = {"Критичний": COLORS["danger"], "Високий": COLORS["warning"],
    ...
    def _build_edit_content(self):
        """Побудова вмісту у режимі редагування"""
        C = COLORS
        rec = self.record
        ...
        def lbl(text, row_idx):
            tk.Label(self.content, text=text, bg=C["bg_dark"], fg=C["text_label"],
                     font=("Segoe UI", 8)).grid(row=row_idx, column=0, sticky="w", padx=16, pady=(6, 0))
        ...
        pri_stat_frame = tk.Frame(self.content, bg=C["bg_dark"])
        pri_stat_frame.grid(row=row, column=0, sticky="w", padx=16, pady=4)
        row += 1

        tk.Label(pri_stat_frame, text="Прiоритет:", bg=C["bg_dark"],
                 fg=C["text_label"], font=("Segoe UI", 8)).grid(row=0, column=0, sticky="w")
        self.e_priority = ttk.Combobox(pri_stat_frame,
        ...
        tk.Label(pri_stat_frame, text="Статус:", bg=C["bg_dark"],
                 fg=C["text_label"], font=("Segoe UI", 8)).grid(row=0, column=1, sticky="w")
        self.e_status = ttk.Combobox(pri_stat_frame,
    ...
    def _save_changes(self):
        ...
        new_record = (
            self.record[0],  # зберігаємо той самий ID
            entity,
            event,
        ...
        # Повернутися до режиму перегляду
        self._cancel_edit()
...
class RegistryTab:
    ...
    def _build_table(self, container):
        ...
        hint_bar = tk.Frame(container, bg=C["bg_dark"])
        hint_bar.grid(row=0, column=1, sticky="e")
        ...
        # Подвійний клік → відкрити вікно деталей
        ...
        # ── Деталі запису (швидкий перегляд) ────────────────────────────────────────────────
    ...
    def _on_double_click(self, event):
        """Відкрити вікно деталей при подвійному кліку"""
        sel = self.tree.selection()
        if not sel:
            return
        # Перевірити, що клік був по рядку, а не по заголовку
    ...
    def _open_selected_detail(self):
        """Відкрити вікно деталей для вибраного запису"""
        sel = self.tree.selection()
        if not sel:
    ...
    def _on_detail_save(self, iid, old_id, new_record):
        """Callback: зберегти зміни з вікна деталей"""
        # Оновити в all_records
        for i, r in enumerate(self.all_records):
            if str(r[0]) == str(old_id) or str(r[0]).lstrip("0") == str(old_id).lstrip("0"):
                self.all_records[i] = new_record
                break
        # Оновити рядок у таблиці
        try:
    ...
    def _on_detail_delete(self, iid, idx_str):
        """Callback: видалити запис з вікна деталей"""
        try:
    ...
    # ── Форма (тільки для нових записів) ─────────────────────────────────────
...
def main():
    root = tk.Tk()
    root.title("Суттєвi подiї — Реєстр v2.1")
