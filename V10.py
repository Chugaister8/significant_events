...
class RiskAppetitePage(tk.Frame):
    ...
    def _show_menu(self) -> None:
        for w in self.winfo_children():
            w.destroy()
        C = COLORS
        self.grid_rowconfigure(0, weight=0)   # title strip
        self.grid_rowconfigure(1, weight=0)   # summary bar
        self.grid_rowconfigure(2, weight=0)   # cards row 1
        self.grid_rowconfigure(3, weight=0)   # cards row 2
        self.grid_rowconfigure(4, weight=1)   # history table (expands)
        self.grid_rowconfigure(5, weight=0)   # hint
        self.grid_columnconfigure(0, weight=1)
        ...
        # ── row 0: title strip ───────────────────────────────────────────
        title_f = tk.Frame(self, bg=C["bg_header"])
        ...
        tk.Label(title_f, text="РИЗИК АПЕТИТ",
            ...
            row=0, column=0, padx=24, pady=(14, 2), sticky="w")
        tk.Label(title_f,
        ...
        # ── row 1: summary bar ───────────────────────────────────────────
        sum_f = tk.Frame(self, bg=C["bg_surface"])
        sum_f.grid(row=1, column=0, columnspan=2, sticky="ew", padx=12, pady=(8, 2))
        tk.Label(sum_f, text=f"Зведення за {cur_period}:",
                 bg=C["bg_surface"], fg=C["text_muted"],
                 font=FONT_SMALL_BOLD).pack(side="left", padx=12, pady=6)
        ...
        # ── rows 2–3: 2×2 cards (fixed height, no scroll) ───────────────
        for i, dr in enumerate(RA_DIRECTIONS):
            r = (i // 2) + 2
            c = i % 2
            self._build_dir_card(self, dr, r, c, cur_period)

        # ── row 4: history table (scrollable internally) ─────────────────
        self._build_history_table(self, 4, cur_period)

        # ── row 5: hint ──────────────────────────────────────────────────
        tk.Label(self,
                 text="Дані зберігаються при натисканні «Зберегти» у кожному напрямку.",
                 bg=C["bg_main"], fg=C["text_subtle"],
                 font=FONT_TINY).grid(
    ...
    def _build_history_table(
        ...
        # Outer container — fills remaining height
        tbl_outer = tk.Frame(parent, bg=C["bg_surface"])
        tbl_outer.grid(row=row + 1, column=0, columnspan=2,
                       sticky="nsew", padx=12, pady=(0, 0))
        ...
        if not all_periods_sorted:
            tk.Label(tbl_outer,
                     text="Немає збережених даних.\nВведіть показники у кожному напрямку.",
                     bg=C["bg_surface"], fg=C["text_subtle"],
                     font=("Arial", 9), justify="center").grid(
                row=0, column=0, pady=30)
            return

        # Horizontal scroll for wide table
        h_canvas = tk.Canvas(tbl_outer, bg=C["bg_surface"],
                              highlightthickness=0)
        h_sb = ttk.Scrollbar(tbl_outer, orient="horizontal",
                              command=h_canvas.xview)
        v_sb = ttk.Scrollbar(tbl_outer, orient="vertical",
        ...
        tbl_f = tk.Frame(h_canvas, bg=C["bg_surface"])
        tcw = h_canvas.create_window((0, 0), window=tbl_f, anchor="nw")

        def _tconf(_: object) -> None:
            h_canvas.configure(scrollregion=h_canvas.bbox("all"))
        tbl_f.bind("<Configure>", _tconf)
        _scroll_mgr.attach(h_canvas)
        ...
        tk.Label(hdr, text="Напрямок", bg=C["bg_surface_alt"],
                 fg=C["text_subtle"], font=FONT_TINY,
                 width=24, anchor="w").pack(side="left", padx=8, pady=5)
        ...
        if len(all_periods_sorted) > 1:
            tk.Frame(tbl_f, bg=C["border_soft"], height=1).pack(fill="x")
            yr_f = tk.Frame(tbl_f, bg=C["bg_surface_alt"])
            yr_f.pack(fill="x")
            tk.Label(yr_f, text="Річний агрегат",
                     bg=C["bg_surface_alt"], fg=C["text_muted"],
                     font=FONT_TINY, width=24, anchor="w").pack(
                side="left", padx=8, pady=4)
            for p in all_periods_sorted:
                yr = p.split()[-1]
                yr_levels: list[str] = []
                for dr in RA_DIRECTIONS:
                    for pp, ent_map in self._all_data.get(dr["key"], {}).items():
                        if pp.split()[-1] == yr:
                ...
                bg_c = RA_COLORS.get(agg, C["bg_surface_alt"]) if agg != "—" \
                       else C["bg_surface_alt"]
                fg_c = "white" if agg != "—" else C["text_subtle"]
                tk.Label(yr_f, text=f" {agg[:3] if agg != '—' else '—'} ",
                         bg=bg_c, fg=fg_c, font=FONT_TINY,
                         width=9, anchor="center").pack(
                    side="left", padx=2, pady=3)
...
class AnalyticsPage(tk.Frame):
    """
    Кросс-модульна аналітика ATLAS.
    Агрегує дані з усіх модулів: реєстр ризиків, суттєві події,
    ризик-апетит, координатори.
    Вкладки: Огляд | Ризики | Події | Ризик-апетит | Координатори
    """
    ...
    def _build(self) -> None:
        ...
        hdr.grid_propagate(False)
        hdr.columnconfigure(0, weight=1)
        tk.Label(hdr, text="АНАЛІТИКА",
                 bg=C["bg_header"], fg=C["accent_muted"],
                 font=FONT_TITLE).grid(row=0, column=0, padx=20, pady=14, sticky="w")
        make_button(hdr, "↻  Оновити все",
                    bg=C["accent"], fg="white",
                    activebackground=C["accent_soft"],
                    font=FONT_BOLD, padx=14, pady=4,
                    command=self._refresh_all).grid(
            row=0, column=1, padx=16, pady=12)
        ...
        tabs = [
            ("  Огляд  ",           self._build_overview_tab),
            ("  Ризики  ",          self._build_risks_tab),
            ("  Події  ",           self._build_events_tab),
            ("  Ризик-апетит  ",    self._build_appetite_tab),
            ("  Координатори  ",    self._build_coords_tab),
        ]
        self._nb = nb
        self._tab_builders: dict[int, tuple] = {}

        for idx, (label, builder) in enumerate(tabs):
            frame = tk.Frame(nb, bg=C["bg_main"])
            frame.rowconfigure(0, weight=1)
            frame.columnconfigure(0, weight=1)
            nb.add(frame, text=label)
            self._tab_builders[idx] = (builder, frame)

        nb.bind("<<NotebookTabChanged>>", self._on_tab_change)
        # Build overview immediately
        self._on_tab_change(None)

    def _on_tab_change(self, _: object) -> None:
        try:
            idx = self._nb.index(self._nb.select())
        except tk.TclError:
            idx = 0
        builder, frame = self._tab_builders.get(idx, (None, None))
        if builder and frame:
    ...
    def _refresh_all(self) -> None:
        self._on_tab_change(None)
        _show_toast(self, "Дані оновлено")

    # ── Data loaders ──────────────────────────────────────────────────────

    @staticmethod
    def _load_risks() -> list[RiskRecord]:
        ...
        try:
            ...
            for item in raw:
                if isinstance(item, dict):
                    result.append(RiskRecord.from_dict(item))
                elif isinstance(item, (list, tuple)):
                    result.append(RiskRecord.from_list(list(item)))
            return result
        except (json.JSONDecodeError, OSError, ValueError):
            return []

    @staticmethod
    def _load_events() -> list[EventRecord]:
        ...
        try:
            ...
            for item in raw:
                if isinstance(item, dict):
                    result.append(EventRecord.from_dict(item))
                elif isinstance(item, (list, tuple)):
                    result.append(EventRecord.from_list(list(item)))
            return result
        except (json.JSONDecodeError, OSError, ValueError):
            return []

    @staticmethod
    def _load_coords() -> list[CoordinatorRecord]:
        if not os.path.exists(COORDS_DATA_FILE):
            return []
        try:
            with open(COORDS_DATA_FILE, "r", encoding="utf-8") as f:
                raw = json.load(f)
            return [CoordinatorRecord.from_dict(i) if isinstance(i, dict)
                    else CoordinatorRecord.from_list(list(i)) for i in raw]
        except (json.JSONDecodeError, OSError, ValueError):
            return []

    @staticmethod
    def _load_appetite() -> dict:
    ...
    @staticmethod
    def _scrollable(parent: tk.Misc) -> tk.Frame:
        C = COLORS
    ...
    @staticmethod
    def _section(parent: tk.Misc, row: int, title: str,
        ...
        f.grid(row=row, column=0, columnspan=4, sticky="ew",
               padx=14, pady=(16, 4))
        tk.Frame(f, bg=col, width=4, height=16).pack(side="left")
        tk.Label(f, text=f"  {title}", bg=C["bg_main"], fg=col,
                 font=FONT_BOLD).pack(side="left")
        return row + 1

    @staticmethod
    def _kpi_cards(parent: tk.Misc, row: int,
                   cards: list[tuple[str, str, str]]) -> int:
        """cards = [(label, value, color), ...]"""
        C  = COLORS
        cf = tk.Frame(parent, bg=C["bg_main"])
        cf.grid(row=row, column=0, columnspan=4,
                sticky="ew", padx=14, pady=(0, 6))
        for i in range(len(cards)):
            cf.columnconfigure(i, weight=1)
        for ci, (lbl, val, color) in enumerate(cards):
            card = tk.Frame(cf, bg=C["bg_surface"], padx=14, pady=10)
            card.grid(row=0, column=ci, padx=5, sticky="nsew")
            tk.Frame(card, bg=color, height=3).pack(fill="x")
            tk.Label(card, text=lbl, bg=C["bg_surface"],
                     fg=C["text_muted"], font=FONT_SMALL).pack(
                anchor="w", pady=(6, 2))
            tk.Label(card, text=val, bg=C["bg_surface"],
                     fg=color, font=FONT_NUMBER).pack(anchor="w")
        return row + 1

    @staticmethod
    def _mini_table(parent: tk.Misc, row: int,
                    headers: list[str], rows: list[list[str]],
                    col_widths: list[int] | None = None) -> int:
        C  = COLORS
        tf = tk.Frame(parent, bg=C["bg_surface"])
        tf.grid(row=row, column=0, columnspan=4,
                sticky="ew", padx=14, pady=(0, 8))
        widths = col_widths or [18] * len(headers)

        hdr = tk.Frame(tf, bg=C["bg_surface_alt"])
        hdr.pack(fill="x")
        for h, w in zip(headers, widths):
            tk.Label(hdr, text=h, bg=C["bg_surface_alt"],
                     fg=C["text_subtle"], font=FONT_TINY,
                     width=w, anchor="w").pack(side="left", padx=4, pady=4)

        for ri, data_row in enumerate(rows[:20]):
            bg = C["row_even"] if ri % 2 == 0 else C["row_odd"]
            rf = tk.Frame(tf, bg=bg)
            rf.pack(fill="x")
            for cell, w in zip(data_row, widths):
                tk.Label(rf, text=str(cell)[:30], bg=bg,
                         fg=C["text_primary"], font=FONT_TINY,
    ...
    @staticmethod
    def _bar_chart(parent: tk.Misc, row: int,
                   data: dict[str, int], color: str,
                   title: str, height: int = 120) -> int:
        C = COLORS
        if not data:
            return row
        cf = tk.Frame(parent, bg=C["bg_surface"])
        cf.grid(row=row, column=0, columnspan=2,
                sticky="ew", padx=14, pady=(0, 8))
        tk.Label(cf, text=title, bg=C["bg_surface"],
                 fg=C["text_muted"], font=FONT_SMALL_BOLD).pack(
        ...
        BOTTOM = 24
        TOP    = 10
        canvas = tk.Canvas(cf, bg=C["bg_surface"],
        ...
        for ci, (label, val) in enumerate(data.items()):
            x0 = LEFT + ci * (avail / n) + gap / 2
            x1 = x0 + bw
            bar_h = (val / max_v) * (H - TOP - BOTTOM)
            y0 = H - BOTTOM - bar_h
            y1 = H - BOTTOM
            ...
            canvas.create_text((x0 + x1) / 2, y0 - 4,
                                text=str(val),
                                fill=C["text_muted"], font=("Arial", 7))
            lbl = label[:10]
            canvas.create_text((x0 + x1) / 2, H - BOTTOM + 10,
                                text=lbl, fill=C["text_muted"],
                                font=("Arial", 6))
        return row + 1

    @staticmethod
    def _donut(parent: tk.Misc, row: int, col: int,
               data: dict[str, int], colors: dict[str, str],
               title: str) -> None:
        C  = COLORS
        cf = tk.Frame(parent, bg=C["bg_surface"])
        cf.grid(row=row, column=col, sticky="nsew", padx=(14 if col == 0 else 4, 4),
                pady=(0, 8))
        tk.Label(cf, text=title, bg=C["bg_surface"],
                 fg=C["text_muted"], font=FONT_SMALL_BOLD).pack(
            anchor="w", padx=8, pady=(6, 2))
        ...
        for label, val in data.items():
            ...
            color  = colors.get(label, C["border_soft"])
            if extent > 0:
                canvas.create_arc(20, 10, W - 20, H - 10,
                                   start=start, extent=extent,
                                   fill=color, outline=C["bg_surface"], width=2)
            start += extent

        # Centre hole
        canvas.create_oval(45, 30, W - 45, H - 30,
                            fill=C["bg_surface"], outline=C["bg_surface"])
        canvas.create_text(W // 2, H // 2,
                            text=str(total),
                            fill=C["text_primary"], font=("Arial", 11, "bold"))

        # Legend
        leg = tk.Frame(cf, bg=C["bg_surface"])
        leg.pack(padx=8, pady=(0, 6))
        for label, val in data.items():
            lf = tk.Frame(leg, bg=C["bg_surface"])
            lf.pack(anchor="w")
            tk.Frame(lf, bg=colors.get(label, C["border_soft"]),
                     width=8, height=8).pack(side="left", padx=(0, 4))
            tk.Label(lf, text=f"{label}: {val}",
                     bg=C["bg_surface"], fg=C["text_muted"],
                     font=FONT_TINY).pack(side="left")

    # ── Tab builders ──────────────────────────────────────────────────────

    def _build_overview_tab(self, frame: tk.Frame) -> None:
        C       = COLORS
        risks   = self._load_risks()
        events  = self._load_events()
        coords  = self._load_coords()
        appetite = self._load_appetite()
        body    = self._scrollable(frame)

        row = 0
        row = self._section(body, row, "Загальний огляд системи")
        row = self._kpi_cards(body, row, [
            ("Реєстр ризиків",        str(len(risks)),   C["accent"]),
            ("Суттєві події",          str(len(events)),  C["accent_warning"]),
            ("Координатори",           str(len(coords)),  C["accent_success"]),
            ("Напрямки ризик-апетиту", str(len([k for k in appetite if appetite[k]])),
             "#A855F7"),
        ])

        row = self._section(body, row, "Ризики за рівнем Score")
        critical = sum(1 for r in risks if r.score.isdigit() and int(r.score) > 16)
        high     = sum(1 for r in risks if r.score.isdigit() and 10 <= int(r.score) <= 16)
        medium   = sum(1 for r in risks if r.score.isdigit() and 5 <= int(r.score) <= 9)
        low      = sum(1 for r in risks if r.score.isdigit() and int(r.score) <= 4)
        row = self._kpi_cards(body, row, [
            ("🔴 Критичний (17–25)", str(critical), C["accent_danger"]),
            ("🟠 Високий (10–16)",   str(high),     "#F97316"),
            ("🟡 Помірний (5–9)",    str(medium),   C["accent_warning"]),
            ("🟢 Низький (1–4)",     str(low),       C["accent_success"]),
        ])

        row = self._section(body, row, "Події за пріоритетом та статусом")
        row = self._kpi_cards(body, row, [
            ("Критичних подій",   str(sum(1 for e in events if e.priority == "Критичний")),
             C["accent_danger"]),
            ("Відкрито / В обробці", str(sum(1 for e in events if e.status in
             ("Вiдкрито", "В обробцi"))), C["accent_warning"]),
            ("Вирішено / Закрито", str(sum(1 for e in events if e.status in
             ("Вирiшено", "Закрито"))), C["accent_success"]),
            ("Координаторів без погодження",
             str(sum(1 for c in coords if c.has_approval != "Так")),
             C["accent_danger"]),
        ])

        row = self._section(body, row, "Топ-10 підприємств за кількістю ризиків")
        ent_counter = Counter(r.entity for r in risks if r.entity)
        if ent_counter:
            trows = [[e, str(c)] for e, c in ent_counter.most_common(10)]
            self._mini_table(body, row, ["Підприємство", "Кількість ризиків"],
                             trows, [36, 14])
        else:
            tk.Label(body, text="Немає даних.", bg=C["bg_main"],
                     fg=C["text_subtle"], font=FONT_SMALL).grid(
                row=row, column=0, padx=14, pady=6, sticky="w")

    def _build_risks_tab(self, frame: tk.Frame) -> None:
        C    = COLORS
        risks = self._load_risks()
        body  = self._scrollable(frame)
        row   = 0
        ...
        # KPI
        total    = len(risks)
        active   = sum(1 for r in risks if r.status == "Активний")
        mitig    = sum(1 for r in risks if r.status == "Мiтигований")
        closed   = sum(1 for r in risks if r.status == "Закрито")
        critical = sum(1 for r in risks if r.score.isdigit() and int(r.score) > 16)

        row = self._section(body, row, "Реєстр ризиків — ключові показники")
        row = self._kpi_cards(body, row, [
            ("Всього ризиків",    str(total),    C["accent"]),
            ("Активних",          str(active),   C["accent_danger"]),
            ("Мітигованих",       str(mitig),    C["accent_warning"]),
            ("Закрито",           str(closed),   C["accent_success"]),
            ("Критичних",         str(critical), "#EF4444"),
        ])

        body.columnconfigure(0, weight=1)
        body.columnconfigure(1, weight=1)

        # Donut: розподіл за типом
        row = self._section(body, row, "Розподіл")
        type_cnt = Counter(r.risk_type for r in risks)
        status_cnt = Counter(r.status for r in risks)
        self._donut(body, row, 0, dict(type_cnt), RISK_COLORS, "За типом ризику")
        self._donut(body, row, 1, dict(status_cnt), {
            "Активний": C["accent_danger"], "Монiторинг": C["accent_warning"],
            "Мiтигований": C["accent"], "Закрито": C["text_muted"],
        }, "За статусом")
        row += 1

        # Bar: top entities
        row = self._section(body, row, "Топ підприємств за кількістю ризиків")
        ent_c = Counter(r.entity for r in risks)
        self._bar_chart(body, row, dict(ent_c.most_common(8)),
                        C["accent"], "Підприємства", height=130)
        row += 1

        # Critical risks table
        crits = [r for r in risks if r.score.isdigit() and int(r.score) > 16]
        if crits:
            row = self._section(body, row, "Критичні ризики (Score > 16)",
                                 C["accent_danger"])
            trows = [[r.id, r.entity[:18], r.risk_name[:28],
                      r.score, r.status] for r in crits]
            self._mini_table(body, row,
                             ["ID", "Підприємство", "Назва ризику", "Score", "Статус"],
                             trows, [5, 18, 28, 6, 12])
    ...
    def _build_events_tab(self, frame: tk.Frame) -> None:
        ...
        total  = len(events)
        opened = sum(1 for e in events if e.status in ("Вiдкрито", "В обробцi"))
        crit   = sum(1 for e in events if e.priority == "Критичний")
        closed = sum(1 for e in events if e.status in ("Закрито", "Вирiшено"))

        row = self._section(body, row, "Реєстр суттєвих подій")
        row = self._kpi_cards(body, row, [
            ("Всього записів",        str(total),  C["accent"]),
            ("Відкрито / В обробці",  str(opened), C["accent_danger"]),
            ("Критичних",             str(crit),   C["accent_warning"]),
            ("Закрито / Вирішено",    str(closed), C["accent_success"]),
        ])
        ...
        row = self._section(body, row, "Розподіл")
        risk_cnt  = Counter(e.risk_type for e in events)
        prior_cnt = Counter(e.priority  for e in events)
        self._donut(body, row, 0, dict(risk_cnt), RISK_COLORS, "За типом ризику")
        self._donut(body, row, 1, dict(prior_cnt), {
            "Критичний": C["accent_danger"], "Високий": C["accent_warning"],
            "Середнiй": C["accent"], "Низький": C["accent_success"],
        }, "За пріоритетом")
        row += 1

        # Bar: by entity
        row = self._section(body, row, "Топ підприємств за кількістю подій")
        ent_c = Counter(e.entity for e in events if e.entity)
        self._bar_chart(body, row, dict(ent_c.most_common(8)),
                        C["accent_warning"], "Підприємства", height=130)
        row += 1

        # Open events table
        opened_list = [e for e in events
                       if e.status in ("Вiдкрито", "В обробцi")]
        if opened_list:
            row = self._section(body, row, "Незакриті події",
                                 C["accent_danger"])
            trows = [[e.id, e.entity[:18], e.event_name[:26],
                      e.priority, e.status] for e in opened_list[:20]]
            self._mini_table(body, row,
                             ["ID", "Підприємство", "Назва події",
                              "Пріоритет", "Статус"],
                             trows, [5, 18, 26, 10, 12])

    def _build_appetite_tab(self, frame: tk.Frame) -> None:
        C        = COLORS
        appetite = self._load_appetite()
        body     = self._scrollable(frame)
        row      = 0

        # Compute aggregates per direction per period
        now      = datetime.now()
        cur_q    = f"Q{(now.month-1)//3+1} {now.year}"

        row = self._section(body, row, "Ризик-апетит — поточний квартал та тренд")

        # KPI row: current quarter per direction
        kpi_data = []
        for dr in RA_DIRECTIONS:
            dir_data = appetite.get(dr["key"], {})
            lvls: list[str] = []
            for ent in dir_data.get(cur_q, {}).values():
                for cd in ent.values():
                    l = cd.get("level", "—")
                    if l != "—":
                        lvls.append(l)
            agg   = aggregate_level(lvls) if lvls else "—"
            color = RA_COLORS.get(agg, C["border_soft"])
            kpi_data.append((
                f'{dr["icon"]} {dr["title"].replace(chr(10), " ")}',
                RA_LABELS.get(agg, "—"), color))
        row = self._kpi_cards(body, row, kpi_data)

        # Collect all periods
        all_periods: set[str] = set()
        for dk_data in appetite.values():
            all_periods.update(dk_data.keys())
        periods = sorted(all_periods, key=lambda p: (p.split()[-1], p.split()[0]))

        if not periods:
            tk.Label(body, text="Немає збережених даних ризик-апетиту.",
                     bg=C["bg_main"], fg=C["text_subtle"],
                     font=FONT_SMALL).grid(row=row, column=0, padx=14, pady=20, sticky="w")
            return

        row = self._section(body, row, "Зведена таблиця по кварталах")

        # Table: direction × period
        tf = tk.Frame(body, bg=C["bg_surface"])
        tf.grid(row=row, column=0, columnspan=4, sticky="ew", padx=14, pady=(0, 8))
        row += 1
        ...
        tk.Label(hdr, text="Напрямок", bg=C["bg_surface_alt"],
                 fg=C["text_subtle"], font=FONT_TINY,
                 width=24, anchor="w").pack(side="left", padx=6, pady=4)
        for p in periods:
            bg_h = C["accent"] if p == cur_q else C["bg_surface_alt"]
            tk.Label(hdr, text=p, bg=bg_h,
                     fg="white" if p == cur_q else C["text_subtle"],
                     font=FONT_TINY, width=9, anchor="center").pack(
        ...
        for ri, dr in enumerate(RA_DIRECTIONS):
            ...
            rf = tk.Frame(tf, bg=bg)
            rf.pack(fill="x")
            tk.Label(rf, text=f'{dr["icon"]} {dr["title"].replace(chr(10), " ")}',
                     bg=bg, fg=C["text_muted"], font=FONT_TINY,
                     width=24, anchor="w").pack(side="left", padx=6, pady=3)
            for p in periods:
                lvls = []
                for ent in appetite.get(dr["key"], {}).get(p, {}).values():
                    for cd in ent.values():
                        l = cd.get("level", "—")
                        if l != "—":
                            lvls.append(l)
                agg  = aggregate_level(lvls) if lvls else "—"
                bg_c = RA_COLORS.get(agg, bg) if agg != "—" else bg
                fg_c = "white" if agg != "—" else C["text_subtle"]
                tk.Label(rf,
                         text=f" {agg[:3] if agg != '—' else '—'} ",
                         bg=bg_c, fg=fg_c, font=FONT_TINY,
                         width=9, anchor="center").pack(side="left", padx=2, pady=3)

        # Trend canvas — all directions on one chart
        row = self._section(body, row, "Динаміка агрегованого рівня")
        if len(periods) > 0:
            lvl_score = {"Green": 0, "Yellow": 1, "Orange": 2, "Red": 3}
            W, H = 640, 130
            LEFT, RIGHT, TOP, BOTTOM = 70, 16, 10, 28
            trend_f = tk.Frame(body, bg=C["bg_surface"])
            trend_f.grid(row=row, column=0, columnspan=4,
                         sticky="ew", padx=14, pady=(0, 8))
            row += 1
            cvs = tk.Canvas(trend_f, bg=C["bg_surface"],
                            width=W, height=H, highlightthickness=0)
            ...
            avail = W - LEFT - RIGHT
            col_w = avail / len(periods) if periods else 1

            for score, label in enumerate(["Green", "Yellow", "Orange", "Red"]):
                y = TOP + H - BOTTOM - (score / 3) * (H - TOP - BOTTOM)
                cvs.create_text(LEFT - 4, y, text=label[:3],
                                fill=RA_COLORS[label], font=("Arial", 6), anchor="e")
                cvs.create_line(LEFT, y, W - RIGHT, y,
                                fill=C["border_soft"], dash=(2, 4))

            for di, dr in enumerate(RA_DIRECTIONS):
                xs, ys = [], []
                for ci, p in enumerate(periods):
                    lvls = []
                    for ent in appetite.get(dr["key"], {}).get(p, {}).values():
                    ...
                    score = lvl_score.get(agg, -1)
                    if score >= 0:
                        xm = LEFT + (ci + 0.5) * col_w
                        ym = (H - BOTTOM) - (score / 3) * (H - TOP - BOTTOM)
                        xs.append(xm); ys.append(ym)
                        cvs.create_oval(xm - 3, ym - 3, xm + 3, ym + 3,
                                         fill=dr["color"], outline="")
                if len(xs) > 1:
                    for i in range(len(xs) - 1):
                        cvs.create_line(xs[i], ys[i], xs[i+1], ys[i+1],
                                         fill=dr["color"], width=1, dash=(3, 2))

            # Period labels
            for ci, p in enumerate(periods):
                cvs.create_text(LEFT + (ci + 0.5) * col_w,
                                 H - BOTTOM + 10,
                                 text=p, fill=C["text_muted"],
                                 font=("Arial", 6))

            # Legend
            leg = tk.Frame(trend_f, bg=C["bg_surface"])
            leg.pack(padx=8, pady=(0, 6))
            for dr in RA_DIRECTIONS:
                lf = tk.Frame(leg, bg=C["bg_surface"])
                lf.pack(side="left", padx=8)
                tk.Frame(lf, bg=dr["color"], width=16, height=3).pack(side="left",
                                                                        padx=(0, 4))
                tk.Label(lf, text=dr["title"].replace("\n", " "),
                         bg=C["bg_surface"], fg=C["text_muted"],
                         font=FONT_TINY).pack(side="left")

    def _build_coords_tab(self, frame: tk.Frame) -> None:
        C      = COLORS
        coords = self._load_coords()
        body   = self._scrollable(frame)
        row    = 0
        ...
        total    = len(coords)
        approved = sum(1 for c in coords if c.has_approval == "Так")
        staff    = sum(1 for c in coords if c.is_staff_unit == "Так")
        concur   = sum(1 for c in coords if c.is_concurrent == "Так")
        ents     = len({c.enterprise for c in coords})

        row = self._section(body, row, "Ризик координатори — зведення")
        row = self._kpi_cards(body, row, [
            ("Всього координаторів", str(total),     C["accent"]),
            ("Підприємств",          str(ents),       C["accent_soft"]),
            ("З погодженням",        str(approved),   C["accent_success"]),
            ("Без погодження",       str(total - approved), C["accent_danger"]),
            ("Штатна одиниця",       str(staff),      C["accent_warning"]),
        ])

        body.columnconfigure(0, weight=1)
        body.columnconfigure(1, weight=1)

        row = self._section(body, row, "Розподіл погодження / суміщення")
        self._donut(body, row, 0,
                    {"Погоджено": approved, "Не погоджено": total - approved},
                    {"Погоджено": C["accent_success"],
                     "Не погоджено": C["accent_danger"]},
                    "Наявність погодження")
        self._donut(body, row, 1,
                    {"Штатна": staff, "Суміщення": concur,
                     "Інше": total - staff - concur},
                    {"Штатна": C["accent"], "Суміщення": C["accent_warning"],
                     "Інше": C["text_muted"]},
                    "Тип призначення")
        row += 1

        row = self._section(body, row, "Кількість координаторів по підприємствах")
        ent_c = Counter(c.enterprise for c in coords)
        self._bar_chart(body, row, dict(ent_c.most_common(10)),
                        C["accent"], "Підприємства", height=130)
        row += 1

        # Without approval table
        no_appr = [c for c in coords if c.has_approval != "Так"]
        if no_appr:
            row = self._section(body, row, "Координатори без погодження",
                                 C["accent_danger"])
            trows = [[c.enterprise[:20], c.full_name[:22],
                      c.phone, c.appointed_date] for c in no_appr]
            self._mini_table(body, row,
                             ["Підприємство", "ПІБ", "Телефон", "Дата прийняття"],
                             trows, [20, 22, 14, 14])
...
class ReportsPage(tk.Frame):
    ...
    - Квартальний звіт ризик-апетиту
    - Зведений звіт по ризиках
    - Звіт по суттєвих подіях
    - Довідка по координаторах
    Попередній перегляд у вікні + Експорт у CSV/TXT
    """
    ...
    def _build(self) -> None:
        ...
        tk.Label(hdr, text="ЗВІТИ",
                 bg=C["bg_header"], fg=C["accent_muted"],
                 font=FONT_TITLE).grid(row=0, column=0, padx=20, pady=14, sticky="w")

        # Period selector in header
        ctrl_f = tk.Frame(hdr, bg=C["bg_header"])
        ctrl_f.grid(row=0, column=2, sticky="e", padx=14, pady=8)
        tk.Label(ctrl_f, text="Квартал:", bg=C["bg_header"],
        ...
        self._period_var = tk.StringVar(value=cur_q)
        period_cb = make_dark_combo(ctrl_f, values=periods, width=10)
        period_cb.set(cur_q)
        period_cb.configure(textvariable=self._period_var)
        period_cb.pack(side="left", padx=(4, 12))

        # Main layout: left panel (report list) + right (preview)
    ...
    def _build_report_list(self, parent: tk.Misc) -> None:
        C = COLORS
        parent.rowconfigure(1, weight=1)
        parent.columnconfigure(0, weight=1)

        tk.Label(parent, text="Доступні звіти",
                 bg=C["bg_surface"], fg=C["text_muted"],
                 font=FONT_SMALL_BOLD).grid(
            row=0, column=0, sticky="ew", padx=0, pady=0, ipady=8, ipadx=12)

        lf = tk.Frame(parent, bg=C["bg_main"])
        lf.grid(row=1, column=0, sticky="nsew")
        lf.columnconfigure(0, weight=1)

        self._reports = [
            {
                "id": "quarterly_appetite",
                "title": "Квартальний звіт\nризик-апетиту",
                "icon": "📊",
                "color": "#4F46E5",
                "desc": "Зведення всіх індикаторів за квартал",
            },
            {
                "id": "risk_summary",
                "title": "Зведений звіт\nпо ризиках",
                "icon": "⚠",
                "color": "#EF4444",
                "desc": "Реєстр ризиків, топ критичних",
            },
            {
                "id": "events_report",
                "title": "Звіт по суттєвих\nподіях",
                "icon": "📋",
                "color": "#F59E0B",
                "desc": "Відкриті та критичні події",
            },
            {
                "id": "coordinators_report",
                "title": "Довідка по\nкоординаторах",
                "icon": "👥",
                "color": "#10B981",
                "desc": "Список координаторів та погодження",
            },
            {
                "id": "annual_summary",
                ...
                "icon": "📅",
                "color": "#A855F7",
                "desc": "Агрегат по всіх напрямках за рік",
            },
        ]

        for ri, rep in enumerate(self._reports):
            card = tk.Frame(lf, bg=C["bg_surface"], cursor="hand2")
            card.grid(row=ri, column=0, sticky="ew", padx=8, pady=(6, 0))
            card.columnconfigure(1, weight=1)
            tk.Frame(card, bg=rep["color"], width=4).grid(
                row=0, column=0, rowspan=2, sticky="ns")
            tk.Label(card,
                     text=f'  {rep["icon"]}  {rep["title"].replace(chr(10), " ")}',
                     bg=C["bg_surface"], fg=C["text_primary"],
                     font=FONT_SMALL_BOLD, anchor="w").grid(
                row=0, column=1, sticky="ew", padx=6, pady=(6, 1))
            tk.Label(card, text=f'  {rep["desc"]}',
                     bg=C["bg_surface"], fg=C["text_subtle"],
                     font=FONT_TINY, anchor="w").grid(
                row=1, column=1, sticky="ew", padx=6, pady=(0, 6))

            def _click(r=rep): self._generate_report(r["id"])

            for w in [card] + list(card.winfo_children()):
                try:
                    w.bind("<Button-1>", lambda _, fn=_click: fn())
                    w.configure(cursor="hand2")
                except tk.TclError:
                    pass

            def _enter(_: object, f=card) -> None:
                f.configure(bg=C["bg_surface_alt"])
            def _leave(_: object, f=card) -> None:
                f.configure(bg=C["bg_surface"])
            card.bind("<Enter>", _enter)
            card.bind("<Leave>", _leave)

    def _build_preview_panel(self, parent: tk.Misc) -> None:
        C = COLORS
        parent.rowconfigure(1, weight=1)
        parent.columnconfigure(0, weight=1)

        toolbar = tk.Frame(parent, bg=C["bg_surface_alt"], height=36)
        toolbar.grid(row=0, column=0, sticky="ew")
        toolbar.grid_propagate(False)

        self._report_title_lbl = tk.Label(
            toolbar, text="Оберіть звіт зі списку",
            bg=C["bg_surface_alt"], fg=C["text_muted"],
            font=FONT_SMALL_BOLD)
        self._report_title_lbl.pack(side="left", padx=14, pady=8)

        make_button(toolbar, "⬇  Експорт CSV",
                    bg=C["accent_success"], fg="white",
                    activebackground="#16a34a",
                    font=FONT_SMALL, padx=10, pady=3,
                    command=self._export_csv).pack(side="right", padx=6, pady=6)

        make_button(toolbar, "⬇  Зберегти TXT",
                    bg=C["bg_surface"], fg=C["text_primary"],
                    activebackground=C["bg_surface_alt"],
                    font=FONT_SMALL, padx=10, pady=3,
                    command=self._export_txt).pack(side="right", padx=(0, 4), pady=6)

        # Text preview
        txt_f = tk.Frame(parent, bg=C["bg_input"])
        txt_f.grid(row=1, column=0, sticky="nsew")
        txt_f.rowconfigure(0, weight=1)
        txt_f.columnconfigure(0, weight=1)
        ...
        self._preview.tag_configure("title",
            foreground=COLORS["accent_muted"], font=("Arial", 11, "bold"))
        self._preview.tag_configure("section",
            foreground=COLORS["accent"], font=("Arial", 9, "bold"))
        self._preview.tag_configure("green",
            foreground=RA_COLORS["Green"])
        self._preview.tag_configure("yellow",
            foreground=RA_COLORS["Yellow"])
        self._preview.tag_configure("orange",
            foreground=RA_COLORS["Orange"])
        self._preview.tag_configure("red",
            foreground=RA_COLORS["Red"])
        self._preview.tag_configure("muted",
            foreground=COLORS["text_subtle"])

        self._current_report_id: str = ""
        self._current_report_lines: list[str] = []
    ...
    def _generate_report(self, report_id: str) -> None:
        ...
        generators = {
            "quarterly_appetite":    self._gen_quarterly_appetite,
            "risk_summary":          self._gen_risk_summary,
            "events_report":         self._gen_events_report,
            "coordinators_report":   self._gen_coordinators_report,
            "annual_summary":        self._gen_annual_summary,
        }
        rep = next((r for r in self._reports if r["id"] == report_id), None)
        if rep:
            self._report_title_lbl.configure(
                text=f'{rep["icon"]}  {rep["title"].replace(chr(10), " ")}  ·  {period}')

        gen = generators.get(report_id)
        if gen:
            lines, segments = gen(period)
            self._current_report_lines = lines
            self._render_preview(segments)

    def _render_preview(self, segments: list[tuple[str, str]]) -> None:
        """segments = [(text, tag), ...]"""
        self._preview.configure(state="normal")
        self._preview.delete("1.0", tk.END)
        for text, tag in segments:
            if tag:
                self._preview.insert(tk.END, text, tag)
            else:
    ...
    def _gen_quarterly_appetite(
        self, period: str,
    ) -> tuple[list[str], list[tuple[str, str]]]:
        appetite = self._load_json(APPETITE_FILE) if os.path.exists(APPETITE_FILE) else {}
        if isinstance(appetite, list):
            appetite = {}
        ...
        def h(txt: str, tag: str = "") -> None:
            lines.append(txt)
            segs.append((txt + "\n", tag))

        h(f"КВАРТАЛЬНИЙ ЗВІТ РИЗИК-АПЕТИТУ", "title")
        h(f"Квартал: {period}   Сформовано: {now_str}\n", "muted")
        h("=" * 70 + "\n", "muted")

        for dr in RA_DIRECTIONS:
            dir_data = appetite.get(dr["key"], {})
            h(f'\n{dr["icon"]}  {dr["title"].replace(chr(10), " ").upper()}\n', "section")
            h("-" * 50 + "\n", "muted")

            indicators = DIRECTION_INDICATORS.get(dr["key"], [])
            period_data = dir_data.get(period, {})
            ents = sorted(period_data.keys()) or ["—"]

            for ent in ents:
                ent_data = period_data.get(ent, {})
                h(f'  Підприємство: {ent}\n', "")

                for spec in indicators:
                    code    = spec["code"]
                    cd      = ent_data.get(code, {})
                    level   = cd.get("level", "—")
                    notes   = cd.get("notes", "")
                    tag     = level.lower() if level in RA_COLORS else "muted"
                    lbl     = RA_LABELS.get(level, "—")
                    line    = f'    {code:<10} {spec["name"]:<35} [{lbl}]'
                    h(line, tag)
                    if notes:
                        h(f'              Примітка: {notes}\n', "muted")
                    else:
            ...
            all_lvls = [
                cd.get("level", "—")
                for ent in period_data.values()
                for cd in ent.values()
                if cd.get("level", "—") != "—"
            ]
            agg = aggregate_level(all_lvls) if all_lvls else "—"
            tag = agg.lower() if agg in RA_COLORS else "muted"
            h(f'\n  ► Агрегований рівень: {RA_LABELS.get(agg, "—")}\n', tag)

        h("\n" + "=" * 70 + "\n", "muted")
        h("Кінець звіту\n", "muted")
        return lines, segs

    def _gen_risk_summary(
        self, period: str,
    ) -> tuple[list[str], list[tuple[str, str]]]:
        raw    = self._load_json(RISK_DATA_FILE)
        risks  = [RiskRecord.from_dict(i) if isinstance(i, dict)
                  else RiskRecord.from_list(list(i)) for i in raw]
        now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
        ...
        h("ЗВЕДЕНИЙ ЗВІТ ПО РИЗИКАХ", "title")
        h(f"Сформовано: {now_str}   Записів у реєстрі: {len(risks)}\n", "muted")
        h("=" * 70 + "\n", "muted")

        # Summary counts
        critical = [r for r in risks if r.score.isdigit() and int(r.score) > 16]
        high_r   = [r for r in risks if r.score.isdigit() and 10 <= int(r.score) <= 16]
        active   = [r for r in risks if r.status == "Активний"]

        h(f"  Критичних (>16):  {len(critical)}\n", "red")
        h(f"  Високих (10-16):  {len(high_r)}\n", "orange")
        h(f"  Активних:         {len(active)}\n", "yellow")
        h(f"  Всього:           {len(risks)}\n", "")
        h("\n", "")
        ...
        if critical:
            ...
            h("-" * 70 + "\n", "muted")
            for r in critical:
                h(f"  [{r.id}] {r.entity}  |  {r.risk_name}\n", "red")
                h(f"       Score: {r.score}  |  Статус: {r.status}"
                  f"  |  Власник: {r.owner}\n", "muted")
            h("\n", "")
        ...
        h("-" * 40 + "\n", "muted")
        type_cnt = Counter(r.risk_type for r in risks)
        for rtype, cnt in sorted(type_cnt.items(), key=lambda x: -x[1]):
            bar = "█" * min(cnt, 30)
            h(f"  {rtype:<25} {cnt:>4}  {bar}\n", "")

        # By entity (top-10)
        h("\nТОП-10 ПІДПРИЄМСТВ ЗА РИЗИКАМИ\n", "section")
        h("-" * 40 + "\n", "muted")
        ent_cnt = Counter(r.entity for r in risks)
        for ent, cnt in ent_cnt.most_common(10):
            h(f"  {ent:<35} {cnt:>3}\n", "")
    ...
    def _gen_events_report(
        self, period: str,
    ) -> tuple[list[str], list[tuple[str, str]]]:
        raw    = self._load_json(DATA_FILE)
        events = [EventRecord.from_dict(i) if isinstance(i, dict)
                  else EventRecord.from_list(list(i)) for i in raw]
        now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
        ...
        h("ЗВІТ ПО СУТТЄВИХ ПОДІЯХ", "title")
        h(f"Сформовано: {now_str}   Записів: {len(events)}\n", "muted")
        h("=" * 70 + "\n", "muted")

        opened = [e for e in events if e.status in ("Вiдкрито", "В обробцi")]
        crit   = [e for e in events if e.priority == "Критичний"]

        h(f"  Відкрито / В обробці: {len(opened)}\n", "red")
        h(f"  Критичних:             {len(crit)}\n", "orange")
        h(f"  Всього:                {len(events)}\n", "")
        h("\n", "")
        ...
        if opened:
            ...
            h("-" * 70 + "\n", "muted")
            for e in opened:
                tag = "red" if e.priority == "Критичний" else "yellow"
                h(f"  [{e.id}] {e.entity}  |  {e.event_name}\n", tag)
                h(f"       Пріоритет: {e.priority}  |  Статус: {e.status}"
                  f"  |  Тип: {e.risk_type}\n", "muted")
                if e.description:
                    h(f"       {e.description[:80]}\n", "muted")
            h("\n", "")

        # By risk type
        h("РОЗПОДІЛ ЗА ТИПОМ РИЗИКУ\n", "section")
        h("-" * 40 + "\n", "muted")
        rc = Counter(e.risk_type for e in events)
        for rtype, cnt in sorted(rc.items(), key=lambda x: -x[1]):
            bar = "█" * min(cnt, 30)
            h(f"  {rtype:<28} {cnt:>3}  {bar}\n", "")

        h("\n" + "=" * 70 + "\n", "muted")
        h("Кінець звіту\n", "muted")
        return lines, segs

    def _gen_coordinators_report(
        self, period: str,
    ) -> tuple[list[str], list[tuple[str, str]]]:
        raw    = self._load_json(COORDS_DATA_FILE)
        coords = [CoordinatorRecord.from_dict(i) if isinstance(i, dict)
                  else CoordinatorRecord.from_list(list(i)) for i in raw]
        now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
        ...
        h("ДОВІДКА ПО РИЗИК-КООРДИНАТОРАХ", "title")
        h(f"Сформовано: {now_str}   Координаторів: {len(coords)}\n", "muted")
        h("=" * 70 + "\n", "muted")

        approved  = sum(1 for c in coords if c.has_approval == "Так")
        no_appr   = len(coords) - approved
        ents_set  = {c.enterprise for c in coords}

        h(f"  Підприємств:    {len(ents_set)}\n", "")
        h(f"  Погоджено:      {approved}\n", "green")
        h(f"  Не погоджено:   {no_appr}\n", "red")
        h("\n", "")

        # By enterprise
        by_ent: dict[str, list[CoordinatorRecord]] = {}
        for c in coords:
            by_ent.setdefault(c.enterprise, []).append(c)

        for ent in sorted(by_ent.keys()):
            h(f"\n{ent.upper()}\n", "section")
            h("-" * 50 + "\n", "muted")
            for c in by_ent[ent]:
                appr_tag = "green" if c.has_approval == "Так" else "red"
                appr_sym = "✔" if c.has_approval == "Так" else "✘"
                h(f"  {c.full_name:<30} {c.phone:<16}", "")
                h(f" [{appr_sym} Погодження]\n", appr_tag)
                h(f"     Посада: {c.main_position}\n", "muted")
                h(f"     Підрозділ: {c.department}  |  Наказ №: {c.order_number}"
                  f"  |  З: {c.appointed_date}\n", "muted")

        # Without approval
        no_appr_list = [c for c in coords if c.has_approval != "Так"]
        if no_appr_list:
            h("\nКООРДИНАТОРИ БЕЗ ПОГОДЖЕННЯ\n", "section")
            h("-" * 50 + "\n", "muted")
            for c in no_appr_list:
                h(f"  {c.enterprise:<25} {c.full_name:<30} {c.phone}\n", "red")

        h("\n" + "=" * 70 + "\n", "muted")
        h("Кінець звіту\n", "muted")
        return lines, segs

    def _gen_annual_summary(
        self, period: str,
    ) -> tuple[list[str], list[tuple[str, str]]]:
        appetite = self._load_json(APPETITE_FILE) if os.path.exists(APPETITE_FILE) else {}
        if isinstance(appetite, list):
        ...
        year    = period.split()[-1] if " " in period else str(datetime.now().year)
        now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
        ...
        h(f"РІЧНЕ ЗВЕДЕННЯ РИЗИКІВ — {year}", "title")
        h(f"Сформовано: {now_str}\n", "muted")
        h("=" * 70 + "\n", "muted")

        quarters = [f"Q{q} {year}" for q in range(1, 5)]

        for dr in RA_DIRECTIONS:
            h(f'\n{dr["icon"]}  {dr["title"].replace(chr(10), " ").upper()}\n', "section")
            h("-" * 50 + "\n", "muted")
            h(f'  {"Квартал":<12}', "muted")

            # Determine all enterprises for this direction this year
            dir_data = appetite.get(dr["key"], {})
            all_year_ents: set[str] = set()
            for q in quarters:
                all_year_ents.update(dir_data.get(q, {}).keys())

            for q in quarters:
                period_data = dir_data.get(q, {})
                all_lvls = [
                    cd.get("level", "—")
                    for ent in period_data.values()
                    for cd in ent.values()
                ...
                agg = aggregate_level(all_lvls) if all_lvls else "—"
                tag = agg.lower() if agg in RA_COLORS else "muted"
                h(f'  {q:<10}  {RA_LABELS.get(agg, "Немає даних")}\n', tag)

            # Year aggregate
            year_lvls = [
                cd.get("level", "—")
                for q in quarters
                for ent in dir_data.get(q, {}).values()
                for cd in ent.values()
                if cd.get("level", "—") != "—"
            ]
            yr_agg = aggregate_level(year_lvls) if year_lvls else "—"
            yr_tag = yr_agg.lower() if yr_agg in RA_COLORS else "muted"
            h(f'\n  ► Річний агрегат: {RA_LABELS.get(yr_agg, "Немає даних")}\n', yr_tag)
        ...
        if risks:
            h("\n\nЗАГАЛЬНИЙ СТАН РЕЄСТРУ РИЗИКІВ\n", "section")
            h("-" * 50 + "\n", "muted")
            cnt = Counter(r.status for r in risks)
            for status, c in cnt.most_common():
                h(f"  {status:<20} {c}\n", "")
    ...
    def _export_csv(self) -> None:
        if not self._current_report_lines:
            messagebox.showinfo("Експорт", "Спочатку сформуйте звіт"); return
        path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV файли", "*.csv")],
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8-sig") as f:
                writer = csv.writer(f)
                for line in self._current_report_lines:
    ...
    def _export_txt(self) -> None:
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write("\n".join(self._current_report_lines))
            _show_toast(self, "TXT збережено")
        except OSError as e:
            messagebox.showerror("Помилка", str(e))
    ...
    def _on_nav_click(self, page_key: PageKey) -> None:
        ...
        if page_key not in self._pages:
            if page_key == "material_events":
                ...
                page = AnalyticsPage(self.content_frame)
            elif page_key == "reports":
                page = ReportsPage(self.content_frame)
            else:
