...
def apply_dark_style(root):
    """Налаштування глобальних ttk-стилів під темну тему"""
...
def make_dark_text(parent, **kwargs):
    """Створення темного текстового поля"""
    C = COLORS
    t = tk.Text(
        parent,
        bg=C["bg_input"],
...
class StatCard(ttk.Frame):
    def __init__(self, parent, title, value, color=None, **kw):
        super().__init__(parent, style="Card.TFrame", padding=16, **kw)
        C = COLORS

        self.configure(relief="flat")

        # Кольорова смуга зверху
        top = tk.Frame(self, bg=color or C["accent"], height=3)
        top.pack(fill="x", side="top")

        tk.Label(self, text=title, bg=C["bg_card"], fg=C["text_muted"],
                 font=("Segoe UI", 8)).pack(anchor="w", pady=(6, 2))

        self._val_lbl = tk.Label(self, text=str(value), bg=C["bg_card"],
...
class RegistryTab:
    def __init__(self, parent, on_data_change=None):
        self.parent = parent
        self.on_data_change = on_data_change  # callback для аналітики
        self.frame = ttk.Frame(parent)
        self.all_records = []
        self.editing_iid = None   # IID рядка, що редагується
        self.editing_idx = None   # Індекс у all_records
    ...
    def _build_ui(self):
        ...
        ent_search = tk.Entry(search_frame,
    ...
    def _build_form(self, container):
        ...
        # Прокручуваний канвас для форми
        ...
        def make_entry(parent, **kw):
            e = tk.Entry(parent, bg=C["bg_input"], fg=C["text_primary"],
                         insertbackground=C["text_primary"],
                         relief="flat", bd=4,
                         highlightthickness=1,
                         highlightbackground=C["border"],
        ...
        tk.Label(form, text="Задiянi пiдроздiли / особи:", bg=C["bg_dark"],
        ...
        for col, (lbl, attr) in enumerate([
            ...
            tk.Label(date_frame, text=lbl, bg=C["bg_dark"],
                     fg=C["text_label"], font=("Segoe UI", 8)).grid(
    ...
    def _build_table(self, container):
        ...
        # Кнопки
        for txt, cmd, bg in [
            ("Редагувати", self._edit_selected, C["warning"]),
            ("Дублювати",  self._duplicate_record, C["bg_card"]),
            ("Видалити",   self._delete_selected, C["danger"]),
        ]:
            tk.Button(toolbar, text=txt, bg=bg, fg="white" if bg != C["bg_card"] else C["text_primary"],
        ...
        cols = ("id", "entity", "event", "risk", "priority", "status", "date", "involved", "desc", "measures")
        ...
        for col, (lbl, attr) in enumerate([
            ...
            sub.grid(row=0, column=col, sticky="nsew", padx=(12 if col == 0 else 4, 4), pady=8)
            sub.columnconfigure(0, weight=1)

            tk.Label(sub, text=lbl, bg=C["bg_panel"],
        ...
        # ── Нижній тулбар (експорт) ───────────────────────────────────────
        exp_bar = tk.Frame(container, bg=C["bg_dark"])
        exp_bar.grid(row=3, column=0, sticky="ew", padx=8, pady=6)
        ...
        try:
            ...
            for row in data:
                if isinstance(row, (list, tuple)) and len(row) >= 8:
                    # Сумісність зі старим форматом (8 полів)
                    if len(row) == 8:
    ...
    def _insert_tree_row(self, data):
        # data: (idx, entity, event, involved, risk, date, desc, measures, detect, priority, status)
        if len(data) < 11:
    ...
    def _get_form_data(self):
        ...
        for val, label in [(detect, "дати виявлення"), (event_d, "дати подiї")]:
            ...
            if not is_valid_date(val):
                messagebox.showwarning("Помилка", f"Неправильний формат {label} (очiкується дд.мм.рррр)")
                return None

        detect  = "" if detect  == "дд.мм.рррр" else detect
        event_d = "" if event_d == "дд.мм.рррр" else event_d

        idx = self.editing_idx if self.editing_idx is not None else len(self.all_records) + 1
        return (
            f"{idx:03d}",
    ...
    def _fill_form(self, data):
        """Заповнити форму даними запису"""
        self._clear_form(silent=True)
        fields_map = [
            (self.ent_entity,   data[1]),
            (self.ent_position, ""),
            (self.ent_reporter, ""),
        ]
        for entry, val in fields_map:
            entry.delete(0, tk.END)
            if val:
                entry.insert(0, val)
                entry.configure(fg=COLORS["text_primary"])

        self.cb_event.set(data[2])
        self.cb_risk.set(data[4] if data[4] != "—" else "")

        # detect date
        detect = data[8] if len(data) > 8 else ""
        self.ent_detect.delete(0, tk.END)
        if detect and detect != "—":
            self.ent_detect.insert(0, detect)
            self.ent_detect.configure(fg=COLORS["text_primary"])
        else:
            add_placeholder(self.ent_detect, "дд.мм.рррр")

        self.ent_event_date.delete(0, tk.END)
        if data[5] and data[5] != "—":
            self.ent_event_date.insert(0, data[5])
            self.ent_event_date.configure(fg=COLORS["text_primary"])
        else:
            add_placeholder(self.ent_event_date, "дд.мм.рррр")

        for txt_widget, val in [
            (self.txt_involved,    data[3]),
            (self.txt_description, data[6]),
            (self.txt_measures,    data[7]),
        ]:
            txt_widget.delete("1.0", tk.END)
            if val and val != "—":
                txt_widget.insert("1.0", val)

        if len(data) > 9:
            self.cb_priority.set(data[9])
        if len(data) > 10:
            self.cb_status.set(data[10])
    ...
    def _clear_form(self, silent=False):
        ...
        self.btn_add.configure(text="Додати запис", bg=COLORS["accent"])
        self.btn_edit_cancel.grid_remove()
    ...
    def _add_record(self):
        ...
        if not data[1] or not data[2]:
            messagebox.showwarning("Обов'язковi поля", "Заповнiть назву пiдприємства та назву подiї")
            return

        if self.editing_iid is not None:
            # Режим редагування
            idx = self.editing_idx - 1
            self.all_records[idx] = data
            self.tree.item(self.editing_iid, values=(
            ...
            if self.on_data_change:
                self.on_data_change(self.all_records)
            self._show_toast("Запис оновлено")
        else:
            # Новий запис
    ...
    def _edit_selected(self):
        ...
        if not sel:
            messagebox.showinfo("Редагування", "Оберiть запис для редагування")
            return
        iid = sel[0]
        ...
        try:
            ...
            return
        # Знайти в all_records
        rec = None
        for r in self.all_records:
            if str(r[0]).lstrip("0") == str(idx) or r[0] == idx_str:
                rec = r
                break
        if not rec:
            return
        ...
        self.btn_add.configure(text="Зберегти змiни", bg=COLORS["warning"])
    ...
    def _show_toast(self, msg):
        ...
        # Позиція — знизу справа від головного вікна
...
class AnalyticsTab:
    ...
    def _build_stat_cards(self):
        C = COLORS
        cards_frame = tk.Frame(self.content, bg=C["bg_dark"])
        ...
        for col, (key, title, val, color) in enumerate(defs):
            card = tk.Frame(cards_frame, bg=C["bg_card"], padx=20, pady=14)
    ...
    def _build_charts_placeholder(self):
        ...
        if HAS_MPL:
            charts_row = tk.Frame(self.content, bg=C["bg_dark"])
            ...
            frame_l = tk.Frame(charts_row, bg=C["bg_card"], padx=8, pady=8)
            ...
            frame_r = tk.Frame(charts_row, bg=C["bg_card"], padx=8, pady=8)
            ...
            # Нижня діаграма — топ-5 підприємств
            ...
            frame_b = tk.Frame(self.content, bg=C["bg_card"], padx=8, pady=8)
    ...
    def refresh(self):
        ...
        if risk_counter:
            ...
            self.ax_left.text(0.5, 0.5, "Немає даних", transform=self.ax_left.transAxes,
        ...
        if status_counter:
            ...
            self.ax_right.text(0.5, 0.5, "Немає даних", transform=self.ax_right.transAxes,
                               ha="center", va="center", color=C["text_muted"])
        ...
        if top5:
            ...
            for bar, val in zip(bars, ent_values):
                self.ax_bottom.text(bar.get_width() + 0.05, bar.get_y() + bar.get_height() / 2,
...
class SettingsTab:
    ...
    def _build_ui(self):
        ...
        self._row(content, 1, "Версiя:", "2.0 — Покращений реєстр", C)
        ...
        tk.Label(content,
                 ...
                 font=("Courier", 9),
                 padx=12, pady=8).grid(row=5, column=0, sticky="w")
...
def main():
    root = tk.Tk()
    root.title("Суттєвi подiї — Реєстр v2.0")
    ...
    # Аналітика (треба передати callback)
    analytics = AnalyticsTab(notebook)

    # Реєстр з callback → аналітика
    registry = RegistryTab(notebook, on_data_change=analytics.update_data)

    # Налаштування
    settings = SettingsTab(notebook)

    notebook.add(registry.get_frame(), text="  Реєстр подiй  ")
    notebook.add(analytics.get_frame(), text="  Аналiтика  ")
    notebook.add(settings.get_frame(), text="  Налаштування  ")
    ...
    time_lbl = tk.Label(statusbar, text="",
                         bg=COLORS["header_bg"], fg=COLORS["text_muted"],
                         font=("Segoe UI", 7), padx=12)
    time_lbl.pack(side="right", pady=4)
    ...
    def autosave():
        registry._save_data()
        status_lbl.configure(text=f"Автозбережено о {datetime.now().strftime('%H:%M:%S')}")
